...
export default function ImageConverter() {
  ...
  const createShareLink = async (): Promise<string> => {
    if (!result) throw new Error("No converted image is available to share.");
    ...
    try {
      ...
      formData.append("resultTitle", "Converted Image");
      formData.append("filename", `converted-image.${extension}`);
  ...
  const copyShareLink = async () => {
    try { const url = await createShareLink(); await navigator.clipboard.writeText(url); setShareMessage("Share link copied."); } catch {}
  };

  const shareResult = async () => {
    try {
      const url = await createShareLink();
      if (navigator.share) { await navigator.share({ title: "Converted Image", text: "Check out this converted image from ToolsGift.", url }); setShareMessage("Share link ready."); }
      else { await navigator.clipboard.writeText(url); setShareMessage("Share link copied."); }
    } catch { setShareMessage("Unable to share this result."); }
  };
            ...
            {result && (
              <div className="mt-4">
                <div className="flex flex-wrap gap-3">
                  ...
                  <button type="button" onClick={shareResult} disabled={shareLoading} className="rounded-xl bg-purple-600 px-5 py-3 font-semibold text-white transition hover:bg-purple-700 disabled:cursor-not-allowed disabled:opacity-60">Share</button>
                </div>
                {shareUrl && (
